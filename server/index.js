...
app.post('/api/auth', (req, res, next) => {
  ...
  const sql = `
    select "u"."userId",
           "u"."username",
           array_remove(array_agg("l"."photoId"), null) as "likedPhotos",
           array_remove(array_agg("b"."photoId"), null) as "savedPhotos"
      from "users" as "u"
...
app.post('/api/photos', authorize, upload.single('photo'), (req, res, next) => {
  ...
  if (!req.file) {
    return next(new ApiError(400, 'A photo is required.'));
  }
  const sql = `
...
app.get('/api/comments', (req, res, next) => {
  ...
  if (!photoId) {
    return next(new ApiError(400, 'Invalid photoId.'));
  }
  const sql = `
    select "c".*,
           to_json(("u"."userId", "u"."username")::"users") as user
...
app.post('/api/comments', authorize, (req, res, next) => {
  ...
  if (!content || !content.toString().trim()) {
    return next(new ApiError(400, 'Comments cannot be empty.'));
  }
  const sql = `
    with "newComment" as (
...
app.post('/api/likes', authorize, (req, res, next) => {
  const photoId = parseInt(req.body.photoId, 10);
  if (!photoId) {
    return next(new ApiError(400, 'A photoId is required.'));
  }
  const sql = `
